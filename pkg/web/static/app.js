...
function updateLoadingProgress(completedStep, activeStep = null) {
  ...
  if (completedStep) {
    const item = document.querySelector(
      `.loading-checklist-item[data-step="${completedStep}"]`,
    );
    if (item) {
  ...
  if (activeStep) {
    const item = document.querySelector(
      `.loading-checklist-item[data-step="${activeStep}"]`,
    );
    if (item) {
...
function displayDependencyGraph(graphData) {
  appLogger.debug(
    "displayDependencyGraph called with",
  ...
  const overlappingEdges = elements.filter(
    (e) => e.data.isOverlapping === true,
  );
  appLogger.debug(
  ...
  const cytoscapeStylesheet = [
    ...
    },
    {
      ...
      style: nodeStyle(
        GRAPH_COLORS.sharedLib,
        GRAPH_COLORS.textWhite,
        "#9d6b99",
      ),
    },
    {
    ...
    },
    {
      ...
      style: {
        ...nodeStyle(
          GRAPH_COLORS.package,
          GRAPH_COLORS.textDark,
          GRAPH_COLORS.borderMedium,
        ),
        shape: "roundrectangle",
        "font-weight": "bold",
        "font-size": "14px",
  ...
  if (isInitialLoad) {
    ...
    appLogger.debug(
      "Creating new cytoscape instance with",
      elements.length,
      "elements",
    );
    ...
    appLogger.debug(
      "Incrementally updating cytoscape with",
      elements.length,
      "elements",
    );
    ...
    const currentEdgeIds = new Set(
      cy
        .edges()
        .map(
          (e) => `${e.data("source")}|${e.data("target")}|${e.data("type")}`,
        ),
    );
    ...
    const edgesToRemove = Array.from(currentEdgeIds).filter(
    ...
    if (elementsToAdd.length > 0) {
      ...
      newNodes.forEach((nodeData) => {
        ...
        if (parent && parent.length > 0) {
          ...
          appLogger.debug(
            `[Cytoscape] Positioned new node ${nodeData.data.id} at parent location`,
          );
        }
      });
    }
    ...
    const nodesChanged =
      nodesToRemove.length > 0 || elementsToAdd.some((e) => !e.data.source);
    ...
    if (nodesChanged) {
      ...
      appLogger.debug(
        "[Cytoscape] Skipping layout - only edges changed, nodes stay in place",
      );
    }
  }
}
    ...
    tooltipTimeout = setTimeout(() => {
      ...
      if (edgeType === "static") {
        ...
        if (symbols && symbols.length > 0) {
          ...
          const more =
            symbols.length > 10 ? ` ... +${symbols.length - 10} more` : "";
...
function showConnectionLostModal() {
  ...
  messageEl.textContent =
    "The backend server is not running. Restart the server to continue.";
...
function subscribeToWorkspaceStatus() {
  ...
  workspaceStatusSource.onmessage = function (event) {
    ...
    try {
      ...
      appLogger.error(
        "Error processing workspace status:",
        e,
        "Raw data:",
        event.data,
      );
...
function enrichGraphWithOverlappingInfo(graph, binaries) {
  ...
  appLogger.debug(
    ...
    Array.from(allOverlappingTargets.keys()),
  );
  ...
  binaries.forEach((binary) => {
    if (binary.kind === "cc_shared_library" && binary.overlappingDeps) {
      ...
      if (overlappingCount > 0) {
        ...
        if (node) {
          node.hasOverlap = true;
          node.overlappingTargets = Object.values(
            binary.overlappingDeps,
          ).flat();
          appLogger.debug(
            "Marked shared library as overlapping:",
            binary.label,
          );
        }
      }
...
async function loadGraphData() {
  ...
  try {
    ...
    if (graphResponse.ok) {
      ...
      if (packageGraph && packageGraph.nodes && packageGraph.nodes.length > 0) {
        ...
        try {
          ...
          const renderedGraph =
            await fetchRenderedGraphFromBackend(currentState);
        ...
        appLogger.warn(
          "Package graph has no nodes or is invalid:",
          packageGraph,
        );
      }
    } else {
    ...
    if (moduleResponse.ok) {
      ...
      try {
        const renderedGraph = await fetchRenderedGraphFromBackend(
...
function setupNavigationFilters() {
  ...
  document
    .querySelectorAll('#ruleTypeMenu input[type="checkbox"]')
    .forEach((checkbox) => {
      checkbox.onchange = () => {
        const selectedTypes = new Set(
          Array.from(
            document.querySelectorAll("#ruleTypeMenu input:checked"),
          ).map((cb) => cb.value),
        );
...
function displayTargetModal(details) {
  ...
  if (details.files && details.files.length > 0) {
    ...
    filesEl.innerHTML =
      '<div class="modal-empty">No files found (may need .d files)</div>';
  }
  ...
  if (details.incomingFileDeps && details.incomingFileDeps.length > 0) {
    ...
    incomingEl.innerHTML =
      '<div class="modal-empty">No incoming dependencies</div>';
  }
  ...
  if (details.outgoingFileDeps && details.outgoingFileDeps.length > 0) {
    ...
    outgoingEl.innerHTML =
      '<div class="modal-empty">No outgoing dependencies</div>';
  }
...
async function fetchRenderedGraphFromBackend(viewState) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Backend lens rendering failed: ${response.statusText} - ${errorText}`,
    );
  }
...
function applyGraphDiff(currentGraph, diff) {
  ...
  const edgeMap = new Map(
    currentGraph.edges.map((e) => [`${e.source}|${e.target}|${e.type}`, e]),
  );
...
viewStateManager.addListener(async (newState) => {
  ...
  try {
    ...
    appLogger.error(
      "[App] Backend lens rendering failed - this is a fatal error",
    );
  }
});
...
window.filterAndRenderNavigationList =
  function filterAndRenderNavigationList() {
    ...
    const ruleTypes =
      filters.ruleTypes ||
      new Set(["cc_binary", "cc_library", "cc_shared_library"]);
